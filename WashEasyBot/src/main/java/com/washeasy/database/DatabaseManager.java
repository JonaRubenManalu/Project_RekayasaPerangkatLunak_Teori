...
public class DatabaseManager {
    ...
    private void createTables() {
        ...
        // [NEW] Tabel keywords — menyimpan keyword chatbot yang bisa dikelola admin
        String sqlKeywords = """
        ...
        // [NEW] Tabel info_kedai — menyimpan jam operasional & lokasi secara dinamis
        String sqlInfoKedai = """
            CREATE TABLE IF NOT EXISTS info_kedai (
                id_info   INTEGER PRIMARY KEY AUTOINCREMENT,
                jam_buka  VARCHAR(20)  NOT NULL DEFAULT '09:00',
                jam_tutup VARCHAR(20)  NOT NULL DEFAULT '22:00',
        ...
        // [NEW] Tabel fasilitas — menyimpan fasilitas layanan yang tersedia
        String sqlFasilitas = """
        ...
        try (Statement stmt = connection.createStatement()) {
            ...
            stmt.execute(sqlKeywords);   // [NEW] tabel keywords dinamis
            stmt.execute(sqlInfoKedai);  // [NEW] tabel info kedai
            stmt.execute(sqlFasilitas);  // [NEW] tabel fasilitas
            System.out.println("[DB] Tabel berhasil disiapkan.");
        } catch (SQLException e) {
            System.err.println("[DB] Gagal membuat tabel: " + e.getMessage());
        }

        // [NEW] Migrasi: tambahkan kolom metode_pengambilan & alamat jika belum ada
        migrateHistoryColumns();
    }

    /** [NEW] Migrasi kolom baru ke tabel history yang sudah ada di DB lama */
    private void migrateHistoryColumns() {
        try (Statement stmt = connection.createStatement()) {
            // Cek kolom metode_pengambilan
            try {
    ...
    private void seedData() {
        try {
            ...
            if (rsInfo.next() && rsInfo.getInt(1) == 0) {
                execute("INSERT INTO info_kedai(jam_buka, jam_tutup, lokasi, id_admin) " +
                        "VALUES('09:00','22:00'," +
                        "'Jl. Dr. Wahidin Sudirohusodo No. 5-25, Kotabaru, Gondokusuman, Kota Yogyakarta, DIY'," +
                        "1)");
